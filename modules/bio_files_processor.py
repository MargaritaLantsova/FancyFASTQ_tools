...
def make_safe_output_path(path: Optional[str], default_name: str) -> str:
    """
    Build a unique output path. If the suggested path exists, append
    a numeric suffix before the extension.
    """
...
def convert_multiline_fasta_to_oneline(
    ...
    FASTA with one sequence per line (header on one line, sequence on next).
    """
    ...
    out_path = make_safe_output_path(output_fasta, "oneline.fasta")
    ...
    while line_idx < len(lines):
        ...
        if _BLAST_SEC_RE.search(line):
            ...
            while line_idx < len(lines):
                ...
                if stripped and not stripped.lower().startswith("description"):
                    break
                line_idx += 1
            ...
            if (
                ...
                row = _SPLIT_COLS.split(lines[line_idx].rstrip("\n").strip())
                if row:
                    descriptions.append(row[0])
        line_idx += 1

    uniq_sorted = sorted(set(descriptions), key=lambda s: s.lower())
    out_path = make_safe_output_path(output_file, "blast_best_hits.txt")
        ...
        for raw_line in in_file:
            ...
            if re.match(r"^\s+CDS\s", line):
                if current:
                    ...
                    if in_translation:
                        current["translation"] = "".join(trans_buffer).replace(
                            " ", ""
                        ).replace("\t", "")
                        in_translation = False
                        trans_buffer = []
...
def select_genes_from_gbk_to_fasta(
    ...
    """
    For each target gene (match by /gene or /locus_tag), write translations of
    n_before and n_after neighbor CDS entries into a FASTA file. Target genes
    themselves are not included.
    """
    if not os.path.exists(input_gbk):
        raise FileNotFoundError(input_gbk)

    # normalize list of target names
    ...
    out_path = make_safe_output_path(output_fasta, "neighbors.fasta")
    with open(out_path, "wt", encoding="utf-8") as out_file:
...
def _cli() -> None:
    ...
    gbk_parser.add_argument(
        ...
        required=True,
        nargs="+",
        help="Comma/space-separated string or multiple args",
    )
    ...
    if args.cmd == "convert-fasta":
        out = convert_multiline_fasta_to_oneline(
            args.input_fasta,
            args.output_fasta,
        )
        print(out)
    elif args.cmd == "parse-blast":
        out = parse_blast_output(
            args.input_file,
            args.output_file,
        )
        print(out)
    elif args.cmd == "gbk-neighbors":
        # accept either several tokens or a single long string
